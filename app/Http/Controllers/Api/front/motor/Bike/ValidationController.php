<?php
namespace App\Http\Controllers\front\motor\Bike;
...
{
    public static function ValidateStaticData($vid,$value="")
    {
        $bFlag = True;
        $aVendor = MasterVendor::where('vid', $vid)->first();
        $aVendorAddon = empty($aVendor->bikeaddons) ? [] : json_decode($aVendor->bikeaddons, true);
        $aVendorPlan = empty($aVendor->bike_plans) ? [] : json_decode($aVendor->bike_plans, true);
        $aDataModel = DataModel::where('userid', Auth::id())->first();
        $aAddon = empty($aDataModel->bikeaddon) ? [] : json_decode($aDataModel->bikeaddon, true);
        $nPlan = empty($aDataModel->bike_plan_type) ? '' : $aDataModel->bike_plan_type;
        //dd($nPlan,$aVendorPlan);
        // if(empty($aAddon)||empty($aDataModel->caraddon))
        // {
        //     $bFlag = True;
        // }
        if (!empty($aDataModel->bikeaddon)) {
