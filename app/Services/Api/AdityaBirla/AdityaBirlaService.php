...
use App\Http\Controllers\front\health\adityabirla\AdityaBirlaController;
...
public static function normalizeAddons($addonInput, $config)
{
    ...
    if (empty($addonInput) || !is_array($addonInput)) {
        return $formattedAddons;
    }

    // 1. Agar Associative Array hai (e.g. ["RISW" => true, "PPNDISC" => true])
    $isAssociative = array_keys($addonInput) !== range(0, count($addonInput) - 1);

    if ($isAssociative) {
        foreach ($addonInput as $key => $val) {
            if ($val === false || $val === 'false' || empty($val)) {
                continue;
            }

            $addonData = $addonMaster[$key] ?? null;
            if (is_array($addonData) && isset($addonData['OPTIONS'])) {
                $formattedAddons[$key] = is_bool($val) ? array_key_first($addonData['OPTIONS']) : (string)$val;
            } else {
                $formattedAddons[$key] = is_bool($val) ? true : (string)$val;
            }
        }
        return $formattedAddons;
    }

    // 2. Agar Flat Indexed Array hai (e.g. ["RISW", "PPNDISC", "RIPW", "3TO1"])
    $i = 0;
    $values = array_values($addonInput);
    ...
    while ($i < count($values)) {
        ...
        if (is_array($addonData) && isset($addonData['OPTIONS'])) {
            ...
            if ($nextValue && isset($addonData['OPTIONS'][$nextValue])) {
                $formattedAddons[$key] = (string) $nextValue;
                $i += 2;
                continue;
            } else {
                $formattedAddons[$key] = (string) array_key_first($addonData['OPTIONS']);
            }
        } else {
            $formattedAddons[$key] = true;
        }
...
public static function buildOptionalCovers($selectedAddons)
{
    ...
    foreach ($selectedAddons as $addonKey => $addonValue) {
        if (empty($addonValue) || $addonValue === false || $addonValue === 'false') {
            continue;
        }

        $addonData = $addonMaster[$addonKey] ?? null;

        // Case 1: Simple Checkbox (CoverPartValue must be empty string)
        if (is_string($addonData)) {
            $optionalCovers[] = [
                "optionalCoverName"  => $addonData,
                "optionalCoverValue" => "1",
                "CoverPartCode"      => (string) $addonKey,
                "CoverPartValue"     => ""
            ];
        }
        // Case 2: Dropdown Option Addon
        elseif (is_array($addonData) && isset($addonData['OPTIONS'])) {
            $selectedCode = is_array($addonValue) ? key($addonValue) : $addonValue;

            if ($selectedCode === true || $selectedCode === '1' || $selectedCode === 'true') {
                $selectedCode = array_key_first($addonData['OPTIONS']);
            }

            $optionalCovers[] = [
                "optionalCoverName"  => $addonData['TITLE'] ?? $addonKey,
                "optionalCoverValue" => "1",
                "CoverPartCode"      => (string) $selectedCode,
                "CoverPartValue"     => (string) $selectedCode
            ];
        }
        // Fallback
        else {
            $optionalCovers[] = [
                "optionalCoverName"  => (string) $addonKey,
                "optionalCoverValue" => "1",
                "CoverPartCode"      => (string) $addonKey,
                "CoverPartValue"     => ""
            ];
...
if ($addons === null) {
    if (!empty($journey->addon)) {
        $dbAddon = is_string($journey->addon) ? json_decode($journey->addon, true) : $journey->addon;
        $addons = $dbAddon['selected'] ?? $dbAddon ?? [];
    }
    if (empty($addons)) {
        $addons = GetCache('cache_selected_addons_' . $userId) ?? [];
    }
}
...
$addons = is_array($addons) ? $addons : [];
            ...
            $normalizedAddons = self::normalizeAddons($addons, $config);
$optionalCovers   = self::buildOptionalCovers($normalizedAddons);
...
// ]);
            $payload = [
                ...
                "PolicyCreationRequest" => [
                    ...
                    "Plan_Code" => $params['plan'] ?? "MassMarket",
...
//             $planCode = "MassMarket";   // ✅ as per vendor
...
//                     "Nominee_Relationship_Code" => $nomineeRelationCode
//                 ];
//             }
...
//                     "SumInsured_Type" => (count($memberArray) > 1) ? "Family Floater" : "Individual",
//                     "Member_Type_Code" => self::getMemberTypeCode($members),
//                     "intermediaryCode" => $config['CREDENTIAL']['INTERMEDIARY_CODE'],
//                     "Policy_Tanure" => (string) $tenure,
//                     "QuoteDate" => now()->format('d/m/Y')
//                 ],
...
//                 "mailingArea" => $permanentAddress['city'] ?? '',
//                 "bankAccountType" => "02",
//                 "bankAccountNo" => $journey->bank_account_no ?? '',
//                 "ifscCode" => $journey->ifsc_code ?? '',
//                 "GSTRegistrationStatus" => "Consumers",
//                 "IsEIAavailable" => 0,
//                 "ApplyEIA" => 0
//             ];
...
//             $kycSource = GetCache('ab_kyc_source_' . $userId) ?? 'HYPERVERGE';

// $isDigiLocker  = ($kycSource === 'DIGILOCKER') ? 'Y' : 'N';
// $isHyperVerge  = ($kycSource === 'HYPERVERGE')  ? 'Y' : 'N';
...
//                 "CKYC_Flag" => "N",
//                 "CKYC_Number" => "",
//                 "Hyper_verge_OPD_Status"   => $isHyperVerge,
//     "Digi_Locker_Verified"     => $isDigiLocker,
...
//                 "panNo" => $journey->pan ?? ''
//             ];
//             $cachedPremium = Cache::get('cache_premium_' . $userId);

//             $finalPremium = $cachedPremium
//                 ?? ($quoteResult['PolCreationRespons']['premiumDetails']['Premium']['FinalPremium'] ?? 0);
//             $txnId = $paymentData['data']['txn_id']
//                 ?? ('TXN' . time());
...
public static function createPolicy($userId, $paymentData = [])
{
    try {
        ...
        // ✅ Addons Extract & Normalize
        $selectedAddonsRaw = json_decode($journey->addon, true) ?? [];
        $selectedAddons = $selectedAddonsRaw['selected'] ?? $selectedAddonsRaw ?? [];
        ...
        $roomCategory = $selectedAddons['RRTO'] ?? 'UPTOSI';
        $normalizedAddons = self::normalizeAddons($selectedAddons, $config);
        $optionalCovers = self::buildOptionalCovers($normalizedAddons);
        ...
        // ✅ 4 Chronic Diseases Question IDs mapping
        $chronicDiseaseMap = [
            "1362474029092023" => "Heart",
            "1362474329092023" => "BP",
            "1362474629092023" => "Asthma",
            "1362475229092023" => "Diabetes",
        ];
        ...
        foreach ($members as $index => $member) {
            ...
            if (!empty($member->ped)) {
                ...
                if (is_array($userPed)) {
                    foreach ($userPed as $pedData) {
                        $questionId = $pedData['question_id'] ?? null;
                        if ($questionId && strtolower($pedData['answer'] ?? '') === 'yes') {
                            $pedMap[$questionId] = [
                                'answer' => 'yes',
                                'extra' => $pedData['extra'] ?? []
                            ];
                            ...
                            if (isset($chronicDiseaseMap[$questionId])) {
                                $dName = !empty($pedData['extra']['disease_name']) 
                                    ? $pedData['extra']['disease_name'] 
                                    : $chronicDiseaseMap[$questionId];
                                $selectedChronicDiseases[] = $dName;
                            }
                        }
                    }
                }
            ...
            // ✅ Dynamic Product Components (Chronic / Non-Chronic handling)
            $productComponents = [
                ["productComponentName" => "SumInsured", "productComponentValue" => (string) $sumInsured],
                ["productComponentName" => "Zone", "productComponentValue" => $zone]
            ];

            if (!empty($selectedChronicDiseases)) {
                // Agar chronic diseases selected hain toh har disease ke liye Chronic add hoga
                foreach (array_unique($selectedChronicDiseases) as $diseaseName) {
                    $productComponents[] = [
                        "productComponentName" => $diseaseName,
                        "productComponentValue" => "Chronic"
                    ];
                }
            } else {
                // Default Non-Chronic
        ...
        // ========== STEP 2: BUILD FINAL POLICY REQUEST ==========
        $clientCreation = [
            "salutation" => ($journey->gender == 'male') ? 'Mr' : 'Ms',
            "firstName" => $journey->kyc_name ?? $journey->name ?? '',
            "lastName" => "",
    ...
    /**
     * Fetch policy document from OmniDocs service
     */
    ...
    public static function initiatePayment($userId, $premium = null)
    {
        try {
            ...
            $payload = [
                ...
                "udf6" => "JSPSKIP",
                "udf7" => $proposalNumber,
                "return_url" => "https://uat.digibima.com/health/vendors/adityabirla/journey",
...
// ], JSON_PRETTY_PRINT);
    ...
    //     public function creatPolicy()
//     {
//         $post = [
//  "ClientCreation"=> [
//         "salutation"=> "Mr",
//         "firstName"=> "Sumantkumar",
//         "middleName"=> "Prakashbhai",
//         "lastName"=> "Patel",
//         "dateofBirth"=> "06/06/1988",
//         "gender"=> "M",
//         "educationalQualification"=> "Professional Course",
//         "pinCode"=> "383410",
//         "uidNo"=> "",
...
//         "nationality"=> "Indian",
//         "occupation"=> "O558",
//         "primaryEmailID"=> "sumant_6688@yahoo.com",
//         "contactMobileNo"=> "9924355555",
//         "stdLandlineNo"=> "",
//         "panNo"=> "BILOW4409H",
//         "passportNumber"=> "",
//         "contactPerson"=> "",
...
//         "ageProof"=> "",
//         "others"=> "",
//         "homeAddressLine1"=> "pruthvipura Sabarkantha Ankala",
//         "homeAddressLine2"=> "",
//         "homeAddressLine3"=> "",
//         "homePinCode"=> "383410",
//         "homeArea"=> "",
...
//     ],
//     "PolicyCreationRequest"=> [
//         "Quotation_Number"=> "Q230007735367",
//         "Product_Code"=> "6212",
//         "Plan_Code"=> "6212100003",
//         "SumInsured_Type"=> "Family Floater",
//         "Policy_Tanure"=> "1",
//         "Member_Type_Code"=> "AH19",
//         "intermediaryCode"=> "2100825",
//         "intermediaryBranchCode"=> "RJ009",
//         "agentSignatureDate"=> "",
//         "Customer_Signature_Date"=> "",
//         "businessSourceChannel"=> "",
//         "BusinessType"=> "New Business",
//         "leadID"=> "AU26005199",
//         "Source_Name"=> "AU Small Finance Bank Limited",
//         "IsPayment"=> "1",
//         "goGreen"=> "0",
//         "QuoteDate"=> "28/03/2024",
//         "RefCode1"=> "271600",
//         "RefCode2"=> "2139",
//         "RefCode6"=> "25004832",
//         "SPID"=> "SP0515595462",
...
//                 "MemberNo"=> "1",
//                 "Salutation"=> "Mr",
//                 "First_Name"=> "Sumantkumar",
//                 "Middle_Name"=> "Prakashbhai",
//                 "Last_Name"=> "Patel",
//                 "Gender"=> "M",
//                 "DateOfBirth"=> "06/06/1988",
//                 "Relation_Code"=> "R001",
...
//                 "weight"=> 90,
//                 "occupation"=> "O558",
//                 "NatureOfDuty"=> "ACCOUNTANT",
//                 "Designation"=> "ACCOUNTANT",
//                 "AnnualIncome"=> 500000,
//                 "EMI"=> 0,
//                 "LoanAccountNumber"=> "",
//                 "LoanPrincipalOutstanding"=> 0,
//                 "EducationQualification"=> "Under Graduate",
//                 "PrimaryMember"=> "Y",
//                 "optionalCovers"=> [
//                     [
...
//                 "personalHabitDetail"=> [],
//                 "Nominee_First_Name"=> "Payalben",
//                 "Nominee_Last_Name"=> "patel",
...
//         "collectionAmount"=> "21127",
//         "collectionRcvdDate"=> "28/03/2024",
//         "collectionMode"=> "Online Collections",
//         "remarks"=> null,
//         "instrumentNumber"=> "185194463725935",
//         "instrumentDate"=> "28/03/2024",
//         "bankName"=> "",
//         "branchName"=> "",
//         "bankLocation"=> "",
//         "micrNo"=> "",
...
//     ]
    ...
    //         $plainText = json_encode($post, JSON_UNESCAPED_SLASHES);

    //         $key = "fdc36aef0618933aebc4508a93245760";
...
//         }
    ...
    //         $encryptedResponse = $responseData['EncryptedResponse'];

    //         $parts = explode('.', $encryptedResponse);
...
//         );
    ...
    //         return [
//             // "raw_response" => $response,
//             "decrypted_response" => $finalResponse
//         ];
//     }
    public function testApi()
    {
        $post = [
            ...
            "PolicyCreationRequest" => [
                "Quotation_Number" => "",
                "Product_Code" => 6212,
                "Plan_Code" => "MassMarket",
                "SumInsured_Type" => "Family Floater",
                "Member_Type_Code" => "M139",
                "AutoRenewal" => "N",
                "intermediaryBranchCode" => "10MHMUM02",
                "agentSignatureDate" => "28/03/2024",
                "Customer_Signature_Date" => "28/03/2024",
                "businessSourceChannel" => "",
                "Source_Name" => "Apollo247",
                "SPID" => "",
                "RefCode1" => "",
                "RefCode2" => "",
                "TCN" => "",
            ...
            "MemObj" => [
                "Member" => [
                    [
                        ...
                        "optionalCovers" => [
                            [
                                "optionalCoverName" => "Per Claim Deductible",
                                "optionalCoverValue" => "1",
                                "CoverPartCode" => "PCDED",
                                "CoverPartValue" => "15000"
                            ]
                        ],
                        "productComponents" => [
                            [
                        ...
                        "exactDiagnosis" => "",
                        "dateOfDiagnosis" => "28/03/2024",
                        "lastDateConsultation" => "28/03/2024",
                        "detailsOfTreatmentGiven" => "",
                        "doctorName" => "",
                        "hospitalName" => "",
                        "phoneNumberHosital" => "",
    ...
    public function startClickApi()
    {

        $post = [
            "workflowId" => "kycVerification_v2",
            "redirectUrl" => "https://www.adityabirlacapital.com",
            "transactionId" => "Q230007735367",
            "inputs" => [
                "fullName" => "anurag pandya",
                "panNumber" => "CUEPP4560L",
                "dob" => "19-07-1996",
                "pepCheck" => "Yes"
            ]
...
// ]);
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://apimuat.abhicl.in/ABHI_MT_KYC/partner/start",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => "POST",
    ...
    public function kycResultApi()
    {

        $post = [
            "transactionId" => "ABHI1775453408260"
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://apimuat.abhicl.in/ABHI_MT_KYC/partner/results",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => $requestBody,
            CURLOPT_HTTPHEADER => [
