...
class ShriramService
{
    ...
    public static function generateBikeQuote(Request $request, $today, $nextyear = "", $nPlanType = null, $idv = null)
    {
        try {
            ...
            if (GetCache($cachemotortype) == "newbike") {
                $aBikedata = json_decode($aData->newbike_reg_details, true);
        ...
        try {
            ...
            $postdata = json_encode([
                "objPolicyEntryETT" => [
                    ...
                    "ElectricalaccessYN" => 0,
                    "ElectricalaccessSI" => array_key_exists('electrical', $aResult) ? $aResult['electrical'] : '',
                    "ElectricalaccessRemarks" => "",
                    "NonElectricalaccessYN" => 0,
                    "NonElectricalaccessSI" => array_key_exists('non-electrical', $aResult) ? $aResult['non-electrical'] : '',
                    "NonElectricalaccessRemarks" => "",
    ...
    public static function generatePrivateCarQuote(Request $request, $today, $nextyear = "", $nPlanType = null, $idv = null)
    {
        try {
            ...
            //         $InvReturnYN = "Y";
            //         $AntiTheftYN = 1;
            //         $PAforUnnamedPassengerYN = 1;
            //         $LLtoPaidDriverYN = 1;
            //         $NoEmpCoverLL = 1;
            //         $SHRIMOTORPROTECTION_YN = 1;
            //         $LimitedTPPDYN = 1;
            //         $VoluntaryExcess = 1;
            //         $PAPaidDriverConductorCleaner = 1;
            ...
            $postJson = json_encode([
                "objPolicyEntryETT" => [
                    ...
                    "PAforUnnamedPassengerSI" => ($PAforUnnamedPassengerYN == 1) ? $PAforUnnamedamount : "",
                    "ElectricalaccessYN" => "Y",
                    "ElectricalaccessSI" => "0",//array_key_exists('electrical', $aResult) ? $aResult['electrical'] : '',
                    "ElectricalaccessRemarks" => "",
                    "NonElectricalaccessYN" => "Y",
                    "NonElectricalaccessSI" => "0", //array_key_exists('non-electrical', $aResult) ? $aResult['non-electrical'] : '',
                    "NonElectricalaccessRemarks" => "",
    ...
    public static function privateCarProposal(Request $request, $today, $nextyear, $aData)
    {
        try {
            ...
            if (!empty($modelSearch)) {
                $searchWords = preg_split('/\s+/', strtoupper(trim($modelSearch)));
                $query = Vehicle_Info::where('MODEL_DESCRIPTION', $aCardata['model']);

                foreach ($searchWords as $word) {
                    $query->where('MODEL_DESCRIPTION', 'LIKE', "%{$word}%");
                }

                $results = $query->select(
                    'id',
                    'VCODE as code',
                    'MODEL_DESCRIPTION as model'
                )->get();
            ...
            $oModel = Shriram_Vehicle_Master::whereIn(
                'VEHICLE_CODE',
                $results->pluck('code')
            )->first();
            //return $oModel;
    ...
    {
        return response()->json("hfghrtyrty");
...
//https://novaapi.shriramgi.com/NovaWS/novaServices/WebAggregator.svc/RestService/getPaymentStatus
        // $curl = curl_init();
        $postdata = json_encode([
