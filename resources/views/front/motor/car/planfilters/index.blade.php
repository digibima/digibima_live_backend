...
<head>
    ...
    @php
        ...
        // dd($accessories);
        $electricalAccess = $data['accessories'][0] ?? '';
        $nonElectricalAccess = $data['accessories'][1] ?? '';
        $cngAccess = $data['accessories'][2] ?? '';

        // dd($electricalAccess, $nonElectricalAccess, $cngAccess);
...
<body class="planlistbg">
    ...
    <section class="filterrow mt-0 px-5">
        <div class="mb-2">
            <a href="{{ session('motortype') == 'newcar' ? route('car.newcar', 'back') : route('car.knowcartwo', 'back') }}"
                class="smlink prev-step mb-2" id="goBackBtn"><span><i class="bi bi-arrow-left"></i></span>Go
                back to Previous</a>
        </div>
        <div class="row insurance-container ">
                        ...
                        <span class="tooltip-text bottom1" id="bottom" style="font-size: 13px;">
                            ...
                            <div class="opt-out">
                                ...
                                <ul>
                                    ...
                                    <li>You already have a PA cover of ₹15 Lakhs (from another vehicle owned by you
                                        or
            ...
            {{-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#pacoverModal">
                    Show Premium Backup
                </button> --}}
        ...
        <div class="container-fluid">
            <div class="row filblock">
                ...
                <div class="col-md-4 col-lg-4 col-xl-3 mb-2">
                    <div class="row">
                        @include('front.motor.car.common.infocar')
                    </div>
                </div>
                            ...
                            <div class="mb-2">
                                ...
                                <label>
                                    <input type="checkbox" name="accessories" id="accessoriescheckbox"
                                        {{ !empty($accessories) ? 'checked' : '' }}>
                                    <span>Additional Accessories</span>
                                </label>
                            </div>

                            <div class="row" id="AccessoriesTypeDiv"
                                style="{{ !empty($accessories) ? 'display: flex;' : 'display: none;' }}">
                                            ...
                                            <div class="addonlist mb-3">
                                                <label>
                                                    <input type="checkbox" id="lpgcngcheckbox" name="lpg_cng"
                                                        {{ !empty($cngAccess['amount']) ? 'checked' : '' }}>
                                                    <span>LPG/CNG</span>
                                                </label>
                                            </div>
                                            <div class="row mt-2" id="fueltypeDiv"
                                                style="{{ !empty($cngAccess['amount']) ? '' : 'display: none;' }}">
                                                <div class="col-md-6">
                                                    <div class="input-container mb-2">
                                                        <select name="fueltype" class="input form-control"
                                                            ...
                                                            <option value="cng"
                                                                {{ isset($cngAccess['type']) && $cngAccess['type'] === 'cng' ? 'selected' : '' }}>
                                                                CNG</option>
                                                            <option value="lpg"
                                                                {{ isset($cngAccess['type']) === 'lpg' ? 'selected' : '' }}>
                                                                LPG</option>
                                                        </select>
...
<script>
    ...
    async function getCarQuoteStream() {
        let aVendor = @json($data['vendor']);
        console.log(aVendor);
        for (var i = 0; i < aVendor.length; i++) {
            //  console.log(aVendor[i]['isActive']);
            if (aVendor[i]['isActive'] == 0) {
                continue;
            }

            const oResponse = await CallAPI("{{ route('car.getcarquote') }}", aVendor[i], "").then((response) => {
                // console.log(response);
                if (response.status == '1') {
                    const data = response.data;
                    data.logo = aVendor[i]['logo'];
                    // console.log(data);
    ...
    async function getCacheCarQuoteStream() {
        $('#addquots').empty();
        let aVendor = @json($data['vendor']);
        for (var i = 0; i < aVendor.length; i++) {
            if (aVendor[i]['isActive'] == 0) {
            ...
            const oResponse = await CallAPI("{{ route('car.getcachecarquote') }}", aVendor[i], "").then((
                response) => {
                // console.log(response);
                if (response.status == '1') {
                    const data = response.data;
    ...
    function AddAccessories(formData) {
        // Debug: log the formData content
        for (let pair of formData.entries()) {
            console.log(`${pair[0]}: ${pair[1]}`);
        }

        CallAPI("{{ route('car.addaccessories') }}", formData, "")
            .then(response => {
    ...
    function validatePaCheckbox() {
        ...
        const changePacover = async () => {
            let aPlaneResponse = "";
            try {
                aPlaneResponse = await CallAPI("{{ route('car.pacoverreason') }}", selectedOptions, "");
            } catch (error) {
                console.error("API error:", error);
            }
    ...
    function updateIdv(value) {
        ...
        let aResponse = CallAPI("{{ route('car.updateidv') }}", idvValue, "").then(response => {
            var status = response.status;
            console.log(status);
            if (status == 1) {
